...
flowchart TD
    ...
    sSave --> d2{"Відповіла про тривожний симптом?"}
    d2 -->|так| flag["Тривожний симптом"]
    d2 -->|ні| d3{"Дійшла до кінця опитувальника?"}
    flag --> d4{"Звернулася до лікаря зараз?"}
    d4 -->|так| okDoc(("Успіх межі безпеки"))
    d4 -->|ні| sGuard(["Стан: попередження прогорнуто, спрацював запобіжник"])
    sGuard --> d3
    ...
    plan --> d6{"Є ризик-орієнтовані пункти?"}
    ...
    d7 -->|ні| dead4{{"Тупик: призначеного немає ні в плані, ні серед невключених"}}
    sBasis --> okMain(("Успіх: спокій за обидві половини"))
    ...
    class start trig
    class entry,quiz,flag,summary,plan screen
    class sect section
    class sSave,sGuard,sLoad,sErr,sShort,sBasis,sResume state
    class okDoc,okMain ok
    class dead1,dead2,dead3,dead4 dead
    linkStyle 3,6,9,12,18,22,26 stroke:#2e7d32,stroke-width:2px
    linkStyle 2,7,10,13,19,23,27 stroke:#c62828,stroke-width:2px
```
...
2. **Є тривожний симптом?** Перериває потік у будь-якій точці, не лише на початку.
3. **Звернулася до лікаря зараз?** Єдина точка, де правильний результат — **вийти
   з продукту**.
4. **Дійшла до кінця?** Тут живе [північна зірка](CLAUDE.md#північна-зірка). Гілка «ні»
...
5. **Локальні дані на місці?** Ціна архітектурного рішення «тільки на пристрої».
6. **Є ризик-орієнтовані пункти?** Гілка «ні» — це не помилка, а
   [П3](research/personas.md#п3--перше-коло) у типовому випадку.
7. **Знайшла те, що їй призначили?** Єдина точка, де MAIN закривається або не закривається.

**Стани** — автозбереження прогресу · попередження прогорнуто (подія
...
- ✅ **Успіх межі безпеки** — пішла до лікаря замість плану.
...
- ⛔ **Призначеного немає ні в плані, ні серед невключених.** Найважливіший тупик у цьому
  файлі: секція показує невключене **з нашої логіки**, а вона прийшла з конкретним
  призначенням від лікаря. Це
  [відкрите питання дерева №3](sitemap.md#відкриті-питання-дерева), і flow показує,
  що воно б'є рівно в main job.
...
flowchart TD
    ...
    d8 -->|так| own[["Секція: Додано вами"]]
    d8 -->|ні| d5{"Хоче побачити, що вже зроблено?"}
    own --> sOwn(["Стан: власний пункт без вердикту й без джерела"])
    sOwn --> d9{"Очікує від нас оцінки, чи треба це їй?"}
    d9 -->|так| dead4{{"Тупик: межу прочитано як недоробку"}}
    d9 -->|ні| okOwn(("Успіх: усе разом, без чужого вердикту"))
    ...
    classDef dead fill:#fae6e6,stroke:#c62828,color:#3d1414
    class start trig
    class plan,summary,mark,quiz,hist screen
    class card section
    class sRecalc,sRecalc2,sMarked,sErr,sEmpty,sApprox,sRound,sUnsaved,sFile,sOwn state
    class okMark,okEdit,okPlan,okHist,okAll,okOwn ok
    class dead3,dead4 dead
    linkStyle 2,6,11,16,20,25,29,31,34 stroke:#2e7d32,stroke-width:2px
    linkStyle 3,7,12,17,21,26,30,32,35 stroke:#c62828,stroke-width:2px
```
...
5. **Хоче побачити історію?** Екран `[?]` — залежить від
   [питання №3 інвентаризації](sitemap.md#відкриті-питання-інвентаризації).
...
8. **Лікар призначив те, чого в плані немає?** Власний пункт лягає окремою секцією
   «Додано вами» — без вердикту, без джерела. Ми ведемо скринінг, не патологію.
9. **Очікує від нас оцінки?** Найтонше місце всієї механіки, див. тупики. (Т7) Цей empty більше не
   глухий кут: він каже, що саме тут з'явиться після першої відмітки, і веде назад у План —
   туди, де ту відмітку й ставлять.

**Стани** — файл результату на пристрої · власний пункт без вердикту · приблизна дата ·
...
- ✅ **Усе разом, без чужого вердикту** — власний пункт стоїть поруч із нашими, і видно,
  що він її.
- ⛔ **Межу прочитано як недоробку.** Вона додала пункт і чекає, що ми скажемо, чи він
  їй потрібен, — а ми не скажемо, бо [немає джерела — немає
  рекомендації](CLAUDE.md#правила-роботи-з-медичним-контентом). Це та сама пастка, що
  з empty-станом «настанови немає» в [E1](#e1--перестати-відчувати-що-на-мені-заробляють):
  межу треба **проговорити**, інакше вона читається як порожнє місце. Лагодиться
  формулюванням, не структурою.
...
3. **Скористалась переходом «відповідь → пункти»?** Раніше гілки «так» не існувало:
   ...
   [Відкрите питання навігації №2](sitemap.md#відкриті-питання-навігації) закрите (Т10):
...
- ✅ **Видно залежність від конкретної відповіді** — перехід від відповіді до
  пунктів, які вона породила, ухвалено в [навігації](sitemap.md#відкриті-питання-навігації)
...
Чотири речі, яких не було видно на статичному дереві:

1. **Головний тупик MAIN — не в плані, а в тому, чого в плані немає.** Жінка приходить
   із конкретним призначенням; секція «розглянули і не включили» показує невключене
   з **нашої** логіки. Якщо її обстеження не потрапило в жоден із трьох переліків,
   продукт мовчить рівно там, де мав відповісти.
2. **Відмітка про проходження ламається на даті.** Точну дату пам'ятають не завжди,
   а без неї перерахунок не працює.
...
4. **Три з п'ятнадцяти «тупиків» насправді не про застряглу людину, а про наш
...
Перші три додані в [`sitemap.md`](sitemap.md#уточнення-після-flows); четверта —
