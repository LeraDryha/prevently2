...
flowchart TD
    ...
    sUnsaved --> mark
    sMarked --> okMark(("Успіх: шлях не починається спочатку"))
    ...
    plan --> d5{"Хоче побачити, що вже зроблено?"}
    ...
    class card section
    class sRecalc,sRecalc2,sMarked,sErr,sEmpty,sApprox,sRound,sUnsaved state
    class okMark,okEdit,okPlan,okHist ok
    class dead3 dead
    linkStyle 2,6,11,17,22,25 stroke:#2e7d32,stroke-width:2px
    linkStyle 3,7,12,18,23,26 stroke:#c62828,stroke-width:2px
```
...
6. **Є хоч одна відмітка?** Перше повернення завжди дає empty. (Т7) Цей empty більше не
   глухий кут: він каже, що саме тут з'явиться після першої відмітки, і веде назад у План —
   туди, де ту відмітку й ставлять.

**Стани** — приблизна дата · консервативний перерахунок від найранішої межі ·
...
3. **R1 упирається у відсутній зворотний зв'язок** «відповідь → пункт плану».
